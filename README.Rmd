...
The `rddtools` attempt to provide a unified approach to using Regression Discontinuity Design in R.
...
This functionality is made accessible throug hthe associated `rdd_data` function,
...
It implements several tools for RDD analysis that were previously unavailable.
...
The each data set is made available as a `data.frame`.
Using the previously discuss `rdd_data()` function we can transform such a `data.frame` to an object of class `rdd_data`, which inherits from `data.frame`.

For instance, we can use the data from the Initiative Nationale du Development Humaine (INDH) a development project in Morocco.
...
The `indh` object is a `data.frame` containing 720 observations (representing individuals) of three variables:
